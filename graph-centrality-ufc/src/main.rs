...
/* This is like the following Python code
    class Fighter:
...
fn main() {
    ...
    add_edge(&mut graph, &fighter_nodes, 3, 0); // Cornor McGregor vs. Dustin Poirier
    add_edge(&mut graph, &fighter_nodes, 3, 2); // Cornor McGregor vs. Jose Aldo
    add_edge(&mut graph, &fighter_nodes, 3, 4); // Cornor McGregor vs. Nate Diaz
    ...
    for (i, &node) in fighter_nodes.iter().enumerate() {
        ...
        match name.as_str() {
            "Conor McGregor" => println!(
                "{} has the lowest centrality because he has fought with all other fighters in the game", name
            ),
            "Dustin Poirier" | "Nate Diaz" => println!(
                "{} has a centrality of {:.2}, implying they had less fights compared to Conor McGregor", name, closeness
            ),
            "Khabib Nurmagomedov" | "Jose Aldo" => println!(
                "{} has the highest centrality of {:.2} as they have fought with the least number of fighters", name, closeness
            ),
            _=> {}
        }
        println!("------------------------");
